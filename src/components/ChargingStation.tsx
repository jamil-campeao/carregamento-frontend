...
export interface ChargingStationData {
  id: string;
  status: "Livre" | "Carregando" | "Ocupado";
...
export function ChargingStation({ station }: ChargingStationProps) {
  ...
  return (
    <Card className="p-4 space-y-3">
      ...
      <Badge variant={getStatusBadgeVariant(station.status)} className="w-full justify-center">
        {station.status}
      </Badge>
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          <div className="mt-1">
            <Progress value={station.batteryLevel} className="w-full" />
            <span className="text-sm">{station.batteryLevel}%</span>
          </div>
        </div>
        
        <div>
          <span className="text-sm text-muted-foreground">Timestamp Lamport:</span>
